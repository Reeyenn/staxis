...
Status: combined pre-release candidate. Clean-install local type, lint/security, full server/client, full integration, migration-chain, and default Turbopack build gates pass; production migration rehearsal, release-environment checks, and production shadow validation remain. Migrations `0387`, `0390`, the separately owned `0404` dependency, and the implemented `0405` queue boundary are integrated in this candidate. The deterministic evaluator, immutable persistence mapper, authenticated scheduled runner, and explicit bounded historical-correction runner are implemented. The bounded active-only Finding Patterns loader is mounted in both the model-synthesis and deterministic-knowledge portfolio routes. The scheduled route is deliberately absent from the production cron schedule, active projection remains database-disabled, and the legacy portfolio runner remains the production projection owner. The combined `/feed` candidate binds aggregate reads to one exact organization receipt with disclosed coverage and treats that reach as read-only; its company-verdict write boundary is the atomic affected-hotel authorization contract, not a company title. Nothing here claims deployment, cutover, or production validation is complete.

## Scope and repository boundaries

This system finds reproducible patterns across hotels governed by one management company. Its architecture keeps the existing per-hotel detector engine authoritative and adds sister-hotel baselines, deterministic portfolio checks, explicit data-quality abstention, scope classification, and portfolio deduplication. The implemented shadow v2 does not yet import or mutate property-local finding rows; that bridge is an explicit cutover prerequisite described below. V2 makes zero AI calls; bounded AI remains only a future, separately enabled downstream option.

The current repository has the following relevant boundaries:

- `src/lib/findings/` is the per-hotel foundation: feeds, known checks, hotel-history baselines, missing-activity detection, materiality, silencing, persistence, and weekly discovery. That foundation remains authoritative for property-local findings. The shadow v2 records independently synthesized local manifestations with `local_finding_id = null`; it does not claim lineage to a property-local finding it did not load.
...
- `src/lib/company/vp-queue-server.ts` and the company queue are consumers of the projection. The combined candidate resolves one organization to an exact, sorted, untruncated all-authorized receipt before applying the 30-hotel work window, returns authorized/attempted/processed/omitted/unavailable hotel counts plus excluded-finding count and a completeness verdict, and withholds the company-wide brief unless coverage is complete. Brief cache identity includes the current authorization and selected-scope hashes, so a transfer or revocation cannot reuse the prior topology's text. The portfolio runner can still gather the full organization, so the future system must not schedule from page loads or silently expose a different population from the one it evaluated.

The narrow structured Finding Patterns read interface is mounted in the two portfolio answer paths. Other narrow interfaces may be added to read property-local findings, property metadata, metric facts, and organization topology, or to support downstream task routing or resolution tracking. This work does not redesign notes, task queues, workflows, or reporting, and it does not build those downstream routing or resolution systems.

The implemented downstream read boundary is deliberately narrower than a general evidence query. Its service-only RPC accepts an account-bound authorization-receipt ID, account ID, time, and fixed result cap—never an organization ID or property list. The database reasserts the live receipt and derives the exact organization/property scope inside the same statement. Both mounted routes expose claims only from a finalized `active` run; a shadow run returns `shadow_only` with zero candidates, and a disabled/no-finalized-run, stale, abstained, incomplete-scope, authorization-change, or signed outage state produces an honest claim-free mounted receipt rather than evidence. Active mode itself remains database-disabled pending the separately reviewed cutover.
...
8. **Time is explicit.** `data_window`, `analysis_window_anchor`, `source_as_of`, `topology_as_of`, `evaluation_at`, and `recorded_at` are distinct. No historical run reads evidence windows, topology, or cohort attributes through an implicit wall clock.
...
11. **Human decisions are separate from machine observations.** The projector may update machine state in `company_findings`, but it cannot overwrite a user's mute, known-problem, resolve, ownership, or resolution metadata. If the canonical affected-hotel lineage changes, a silence is rearmed as an updated finding; it cannot silently move or expand onto hotels the original actor was not authorized to govern.
...
Every evaluation declares the instant at which organization membership and cohort attributes are interpreted. The implemented scheduled runner uses the stable weekly boundary for `analysis_window_anchor`, `evaluation_at`, `source_as_of`, and `topology_as_of`; a delayed invocation cannot move those boundaries. Supply comparisons additionally require one relationship and compatible profile whose audited validity covers the entire three-complete-local-month evidence interval. Activity would use the complete prior local business day, but v2 marks all current mutable activity sources ineligible, as described below.

Run-window identity is property-independent. Version 3 derives a fixed half-open evidence envelope from policy, not from whichever rows happened to be present: its start is the earlier of the three-month local-calendar start converted with the maximum supported positive UTC offset (`+14`) and the 98-day activity horizon plus a four-day timezone/cutoff safety allowance; its end is one millisecond after the frozen weekly anchor. Input construction rejects any numerator, denominator, or local manifestation outside that envelope. A sparse parent and a corrected backfill therefore retain byte-identical run boundaries instead of changing identity when late facts appear.

`runManagementPatternBackfill` is the executable, projection-neutral correction seam for one historical weekly epoch. Its trusted server-side caller must supply the exact `organizationId`, weekly `analysisWindowAnchor`, later `sourceAsOf`, later decision-time `evaluationAt`, and immutable `supersedesRunId`; there is no date-range sweep or implicit wall-clock evidence boundary. The application requires `analysisWindowAnchor <= sourceAsOf <= evaluationAt`, an exact scheduled weekly anchor, and a maximum 366-day correction horizon. The database independently requires the named parent to belong to the same organization, be terminal, describe the same topology and evidence window, precede the correction monotonically, and—unless it failed—have its completed result-batch receipt. A backfill always uses the historical anchor as `topology_as_of`, writes a distinct `backfill:` run key and immutable supersession link, and remains shadow-only. The original source-as-of receipt is unchanged; a later correction may see a source revision that the original receipt correctly excluded.
...
Region/portfolio assignment and group-definition state use the same canonical transaction-time rule: an unchanged row valid at the cutoff wins; otherwise the unique latest event at/before the cutoff supplies its `after_state`, or the unique earliest event after it supplies its `before_state`. Distinct assignment targets may move North→South at the same transaction instant; a same-target causal tie, unlogged mutation, overlapping duplicate assignment, or backdated validity correction abstains. Exact assignment/group IDs, validity, definition, state timestamps, and proof kind/instant are retained in the property receipt. Later rename or deletion cannot leak into the older cohort, while the later topology sees the successor group.

Profile corrections are append-only knowledge revisions. A correction may reuse an earlier `effective_from`, but must take the next property revision, receives a database-owned strictly increasing `created_at`, resolves only after that knowledge cutoff, and is capped by audited relationship loss. The insert guard accepts an exact historical relationship proof after a transfer but rejects an unbounded predecessor revision, causal audit tie, skipped version, or mismatched tenant/property/relationship identity.

Metric freshness likewise separates domain time from knowledge time. Supply `fresh_through` is the final millisecond of the latest included closed month; rooms-sold `fresh_through` is the end of the latest included business date. `max_closed_at`, `max_sealed_at`, and `max_updated_at` remain separate lifecycle watermarks. The finalizer derives all of them, plus exact signed freshness ages, from the persisted fact rows and rejects caller-declared drift, so a late correction cannot make an old coverage window appear current.
...
Late data or corrections create a new run/evaluation revision with `supersedes_id` and source hashes. Implemented backfills are always projection-neutral. Any future authorization to reconcile a correction into a current projection requires a separate cutover decision and the same fenced, versioned rules as a scheduled run; it must record that a historical revision caused the change.
...
Coverage is computed separately for each target hotel and each fallback rung. Its denominator is every other hotel that could still be compatible at that rung: a missing profile dimension cannot disappear from the denominator, while a known mismatch on a still-active dimension proves incompatibility. A hotel ruled out only by a dimension that the declared fallback just relaxed re-enters the possible-peer population at that rung. The receipt fingerprints the target, active dimensions, possible peers, usable peers, excluded peers, counts, and ratio, so five observed peers cannot be presented as 100% coverage when additional unknown peers may exist.
...
Each accepted DTO matches `portfolio-finding.v1`: exact evaluated and affected hotels, the verified eligible-hotel count, named-property privacy, supported assertion and single direction, analysis window, metric/query/source versions, an evidence-graph fingerprint, and a bounded active lifetime. The immutable evidence graph retains the exact eligible-hotel IDs; the prompt DTO intentionally minimizes them after proving that every eligible hotel is inside the selected receipt. The loader caps output at 40, reports limit omissions and candidate rejections, and records selected run-snapshot included/excluded/missing counts plus at most 50 aggregated exclusion reason codes. It never truncates authorization or selected scope. Both mounted routes validate the producer fingerprint and every DTO against the current receipt and reassert access around the read. The model route may project accepted active claims into its prompt/presentation catalog; the deterministic-knowledge route deliberately applies a zero finding-projection budget, records the mounted producer receipt, and licenses no finding claim in its answer. Empty and mounted-zero findings remain normal states.
...
## Implemented v2 budgets and unvalidated targets
...
| Resource | Implemented v2 behavior |
| --- | --- |
...
| AI | Exactly zero calls, zero tokens, and zero model spend. The run manifest and quality receipt record those zeros. No model or prompt is enabled in this candidate. |
...
This combined candidate implements the additive schema, strict source contract, deterministic evaluator, immutable writer, authenticated shadow endpoint, and a bounded active-only structured-output loader mounted in both portfolio answer routes. It also integrates `0404` and implements the `0405` company-verdict transaction. The endpoint is intentionally absent from `vercel.json`; the database claim rejects active mode; the projector remains cutover-disabled; shadow/disabled/outage states produce mounted zero claims; and the legacy runner remains the only production projection owner. This is not a release or deployment claim: clean-install local automated, real PGlite authorization/concurrency, migration-chain, security/lint, and default Turbopack gates pass, while release-environment checks, production migration rehearsal, and production-safe-window gates remain. No code in this candidate authorizes active projection. Migration `0387` intentionally installs before `0390`; its private typed profile accessor dynamically resolves the later profile table and returns no profile while that table is absent, so that migration gap abstains instead of widening a cohort or failing the migration chain.
...
- The present source tables cannot prove immutable, deletion-resistant activity coverage. V2 therefore produces explicit activity abstentions; a 98-day immutable coverage ledger and its accumulation period are prerequisites for enabling absence detection.
- The shadow v2 does not ingest authoritative property-local finding outputs, so its local manifestations have null finding links. Active projection requires the scoped bridge and lineage validation described in the rollout plan.
- The structured portfolio-finding loader is mounted in both portfolio answer paths, but the database still cannot create an active projection. Shadow, disabled/no-finalized-run, and bounded outage states therefore remain honest mounted-zero receipts with no prompt or answer claims. A future active-read cutover must keep the receipt assertions and separately satisfy the property-local lineage prerequisite; the completed mount does not activate shadow evidence.
...
- A group assignment created or validity-corrected after the frozen topology cutoff but backdated across that cutoff deliberately abstains. V2 does not invent a separate group source-time correction model; an ordinary future-dated move remains exactly reconstructable.
...
- Company-level `queueAvailable` and company titles authorize only the explicit-organization aggregate `/feed` read. The exact untruncated authorization population is retained even when work is bounded to 30 hotels; the response counts attempted, processed, omitted, and unavailable hotels plus findings excluded for unsafe lineage, and marks queue coverage incomplete on any gap. Those reads cannot license a company-wide brief, and the daily brief cache is bound to the current authorization/scope epoch. Aggregate reach never grants a verdict.
- Company-card verdicts are governed only by the implemented `0405` transaction. It locks the tenant/finding and canonical affected-hotel lineage, treats the aggregate receipt only as read provenance, mints and reasserts an exact affected-hotel receipt, requires current mutation standing and topology at every affected hotel, applies the strict Staxis-section rule and a closed detector/family/action capability policy under locks, compare-and-swaps `verdict_revision`, and appends the immutable audit event atomically. Lost-response retries bind to the exact audited lineage; detector lineage changes rearm prior silences. Generic denials leave every row unchanged, and an actor-scoped pre-receipt rate limit bounds retry/storage amplification. Local transaction, concurrency, stale/revoked-authority, and anti-enumeration tests pass; production migration rehearsal and release-environment gates remain.
...
- Migration `0404` remains separately owned but its certified dependency commit is integrated here; the full combined gates and production migration rehearsal remain prerequisites for any release decision.
- Production shadow measurements, dashboards, alert rules, migration rehearsal, and human false-positive review have not yet happened. This is a shadow candidate, not evidence of production readiness by itself.
...
- A future FX conversion would introduce rate-source and timing assumptions. V2 avoids that uncertainty by abstaining outside its reviewed USD policy.
...
- A future AI explanation could be wrong or overconfident even with bounded evidence. No AI path is enabled in v2; if added, it must remain advisory and unable to alter facts or gates.
