...
Status: implemented replacement shadow v2 candidate, pending certified authoritative-access integration, combined migration review, and production shadow validation. Migrations `0389` and `0392`, the deterministic evaluator, immutable persistence mapper, the authenticated scheduled runner, and an explicit bounded historical-correction runner are implemented on this branch. The route is deliberately absent from the production cron schedule and active projection is database-disabled. The legacy portfolio runner remains the production projection owner; its `/feed` boundary reports bounded coverage explicitly but its final authorization semantics are owned by the combined Access/VP integration and this branch must not be deployed alone. This document distinguishes that legacy behavior from future cutover work; it does not claim literal mathematical perfection or production validation that has not happened.
...
- Company-level `queueAvailable`/Owner/VP standing authorizes aggregate `/feed` reads only; it does not authorize mutations of each hotel's private operational findings. This branch still carries the superseded standalone POST implementation and its legacy write-policy tests solely as a semantic-merge input. It is non-governing and must not be released: the combined Access/VP integration must require explicit organization selection plus fresh, exact per-finding/per-hotel action capability inside an all-or-nothing commit-time boundary, with generic anti-enumerating denial and unchanged rows on any unauthorized, stale, or revoked target.
