...
Migration 0378 introduces one per-account authority state:
...
For `/feed`, `queueAvailable` is only the fresh organization-wide read verdict: every hotel in the exact receipt must be covered by company/organization provenance, so a region, portfolio, selected-hotel, or mixed broad+narrow grant cannot expose opaque company cards. Migration 0407 is the sole company-card verdict boundary. Its service-only RPC locks the finding, resolves the finding's exact non-empty affected-hotel set, reasserts a fresh property-subset receipt, requires current `hotelMutationAllowed` plus the closed action-specific section/capability policy at every affected hotel, compares the caller's status/timestamp/revision token, commits once, and appends an immutable audit event. Company Owner/VP authority therefore permits aggregate reads but never manufactures hotel mutation authority. Unauthorized, transferred, stale, mixed-authority, or direct-ID attempts fail without changing the row or revealing which target failed. Hotel-local cards climbed into the company queue remain read-only there until their hotel route has the same transactional receipt/CAS boundary.
...
The legacy physical conversation archive deletes live rows. That would destroy portfolio turn links and null immutable artifact/receipt conversation bindings. Migration 0401 therefore rejects archive and restore for portfolio conversations before any write, while preserving property-chat archive behavior. Receipt-provenance-preserving portfolio archival is intentionally deferred; silently restoring an unreceipted transcript is not allowed.
...
One content-addressed projection is reused by the prompt, presentation catalog, plan validator, deterministic renderer, numeric allowlist, and durable receipt. The producer load is capped at 40 candidates; the shared user-visible projection admits at most 10 Finding claims and is reduced further when necessary so the complete presentation catalog remains at most 64 claims. Both the Finding prompt block and the presentation-plan contract are independently capped at 12,000 UTF-8 bytes; the compact durable projection receipt is capped at 65,536 bytes. The model selects opaque claim IDs only. Rejected, item/UTF-8-budget omitted, and merely projected-but-unselected findings cannot license answer numbers. The compact receipt preserves exact source partitions, status/run provenance, the producer fingerprint, pre-model and post-model omission sets, exact displayed claim IDs, and a recomputable projection hash; it does not retain producer candidate IDs, full authorized hotel IDs, statements, or full finding DTOs. `loaded` with zero accepted claims is valid when every loaded candidate is rejected by either the producer or consumer, while every non-loaded status remains claim-free. Tests prove the same projection/claim IDs flow unchanged through prompt, catalog, plan validation, deterministic rendering, numeric allowlisting, both artifact kinds, and the query receipt, including multibyte UTF-8, rejected, truncated, projected-but-unselected, mounted-zero, and out-of-scope numeric poison cases.

The combined route mounts the independently reviewed loader after selector-specific receipt preparation and accepts only reconciliation-selected `projection_mode = 'active'` candidates. Shadow candidates never cross the consumer boundary. The loader's mounted-zero states (`shadow_only`, `stale`, `abstained`, `no_finalized_run`, `no_applicable`, `incomplete_scope`, `scope_changed`, and `unavailable`) remain claim-free but retain bounded producer/query/run/coverage/truncation/outage provenance. Missing, empty, rejected, or unavailable findings do not block factual portfolio questions.
...
- The Finding Patterns engine remains shadow-only until a separately governed per-organization active-projection cutover. Its loader is mounted safely, so shadow-only currently produces an honest claim-free receipt rather than user-visible pattern prose.
...
- Portfolio conversations are not physically archived/restored until archive storage can preserve the immutable artifact → receipt → committed-message proof graph; 0401 rejects both operations without data loss.
...
- Migration 0406 keeps `company_knowledge` as the compatible current projection while recording append-only, hash-chained organization revisions behind an optimistic current-revision/CAS mutation RPC. Compatibility mode supports either deployment order; enforcement is finalized only after all writers use the ledger-aware contract.
- Company queue verdicts are transactional through 0407, but climbed hotel-local cards remain read-only from the company surface until their property route gains an equivalent final-receipt/CAS commit boundary.
