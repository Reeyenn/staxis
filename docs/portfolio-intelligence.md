...
- Status: combined pre-release candidate; the bounded active-only Finding Patterns loader is mounted in both portfolio answer paths, `0404` is integrated, and the atomic `0405` queue boundary is implemented. Clean-install local full-test, integration, lint/security, migration-chain, and default Turbopack gates pass; production migration rehearsal, release-environment validation, cutover, and deployment remain.
...
For `/feed`, aggregate read authority is bound to one explicit organization and one fresh, exact all-authorized receipt. `queueAvailable` requires company/organization provenance over every hotel in that receipt, so a region, portfolio, selected-hotel, or mixed broad+narrow grant cannot expose opaque company cards. The 30-hotel work window is not an authorization truncation: the response retains the exact authorized count, discloses attempted, processed, omitted, and unavailable hotel counts plus findings excluded for unsafe lineage, marks coverage incomplete on any gap or check outage, and withholds the company-wide brief unless coverage is complete. The daily brief cache is also keyed and envelope-validated against the current authorization and scope hashes, so transfers or revocations cannot reuse old text.

That aggregate verdict is read-only and no company role—owner, VP, or finance—grants a write. In the combined candidate, a company-card verdict is governed by the implemented Migration `0405` RPC: it locks the tenant/finding and its canonical affected-hotel lineage, asserts the aggregate receipt only as read provenance, mints and reasserts an exact affected-hotel receipt, rechecks current per-hotel mutation standing and topology, enforces the strict Staxis-section rule and closed detector/family/action capabilities under locks, compare-and-swaps the expected verdict revision, and appends an immutable audit event in the same transaction. Lost-response retries match the exact audited target array; a detector target shift rearms a prior silence instead of extending it. An actor-scoped pre-receipt rate limit bounds retry/storage amplification. UI affordances are non-authoritative hints; generic denial leaves all rows unchanged. This remains a candidate pending full gates, not a release or deployment claim.
...
The bounded active-only producer loader is mounted in both portfolio routes. The model-synthesis path may project only accepted claims from a fresh finalized `active` run into its prompt, presentation catalog, numeric allowlist, artifacts, and query receipt. The deterministic-knowledge path also loads and fingerprints the producer artifact, but applies a zero finding-projection budget: it records the mounted receipt while licensing no Finding Patterns claim in the deterministic answer. Shadow, database-disabled/no-finalized-run, stale, abstained, incomplete-scope, scope-change, and signed bounded-outage states are honest mounted-zero receipts and expose no claims. A malformed artifact, fingerprint mismatch, or loader that cannot settle inside the hard deadline fails the request closed rather than fabricating a zero state. Valid missing, empty, rejected, or unavailable mounted-zero findings do not block otherwise factual portfolio questions. Active projection remains database-disabled pending cutover.
...
Portfolio Intelligence chat is intentionally read-only. It cannot approve or execute a property or cross-hotel action. The separate human `/feed` company-verdict path is not a model tool and, in this candidate, is limited to the atomic `0405` contract described above. Organization-wide announcements are also disabled until they have the same safe-action contract. A future cross-hotel action protocol must be a separate contract with a deterministic preview, exact target hotel IDs, typed per-hotel operations, explicit user confirmation, authorization recheck at commit, idempotency key, immutable audit event, and per-hotel partial-failure result. Granting an autonomous mutation tool to portfolio synthesis is out of scope.
...
- The independently owned Finding Patterns producer is mounted through its bounded active-only consumer in both portfolio paths, but active projection remains database-disabled. The mount therefore proves integration behavior, not production claim availability or cutover readiness; shadow/disabled/outage states remain claim-free.
- Portfolio-chat cross-hotel mutations are intentionally absent. The separate human `/feed` company-verdict path uses the implemented `0405` atomic affected-hotel receipt/standing/topology/section/capability/CAS/audit boundary and is not release-ready until its combined migration and concurrency/revocation gates pass.
...
- Migration `0404`, the separately owned knowledge revision ledger, is integrated from its certified dependency commit. The local combined migration chain and automated/real PGlite suites pass; the candidate must not be represented as released or deployable until release-environment evaluation and production migration rehearsal have passed.
