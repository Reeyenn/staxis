...
/**
 * Read the enable flag at call time (NOT at module load) so tests can
 * flip `process.env.CUA_CRITIC_ENABLED` between cases. env.ts already
 * validated at boot that the value is one of 'true' / 'false' or unset
 * (defaults to enabled).
 */
function isCriticEnabled(): boolean {
  return process.env.CUA_CRITIC_ENABLED !== 'false';
}
