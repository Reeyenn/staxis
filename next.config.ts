...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      ...
      {
        // Self-hosted ElevenLabs AudioWorklet processors. Static, ~3KB
        // each, rev with each @elevenlabs/client bump (regenerated by
        // scripts/extract-elevenlabs-worklets.mjs). Long-cache so they
        // don't re-fetch on every voice-mode open.
