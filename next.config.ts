...
function buildCsp(): string {
  ...
  return [
    ...
    // media-src allows same-origin blob: audio (e.g. comms voice-message
    // playback via URL.createObjectURL). blob: is same-origin only — it
    // can't mint cross-origin URLs — so this doesn't broaden the attack
    // surface.
    `media-src 'self' blob:`,
    ...
    // worker-src governs Web Worker + AudioWorklet module sources. blob: is
    // permitted so any locally-generated workers keep working.
...
const securityHeaders = [
  ...
  // only (the Ask-bar talk-to-type dictation + comms voice messages need
  // this). Without `microphone=(self)` Chrome refuses every getUserMedia()
  // call on getstaxis.com regardless of the user's site permissions, because the
