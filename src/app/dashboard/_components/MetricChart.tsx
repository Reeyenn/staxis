...
export const MetricChart = React.memo(function MetricChart({ series, color, onHover, marker }: {
  ...
  // Draw-in runs only when the user switches range/metric mid-session — the
  // first paint renders the line complete (no load-time motion, app-wide rule).
  const drewOnceRef = useRef(false);
  useEffect(() => {
    const p = pathRef.current;
    if (!p) return;
    if (!drewOnceRef.current) { drewOnceRef.current = true; return; }
