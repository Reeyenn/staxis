...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Plan v4 bridge — was plan_snapshots.rooms (dropped). Now read
    // from today_room_work_v1 RPC which derives the per-room work list
    // live from pms_room_status_log + pms_reservations.
    const [propRes, workRes] = await Promise.all([
      supabaseAdmin.from('properties').select('name').eq('id', pid).maybeSingle(),
      supabaseAdmin.rpc('today_room_work_v1', { p_property_id: pid, p_date: shiftDate }),
    ]);
    ...
    type WorkRow = { room_number: string; stay_type: 'C/O' | 'Stay' | null };
    const planRooms: PlanRoom[] | null = Array.isArray(workRes.data)
      ? (workRes.data as WorkRow[]).map(r => ({ number: r.room_number, stayType: r.stay_type }))
      : null;
