...
export function decideDoctorCheckAlert(report: DoctorCheckSummary): AlertDecision {
  ...
  // so Sentry/email/Slack lists are scannable. Round 18 (2026-05-15):
  // delegated to truncateListForSentryTitle so doctor + walkthrough alerts
  // share one truncation policy (SENTRY_TITLE_MAX = 180) and one
  // overflow shape (", +N more"). Operators see the same idiom everywhere.
  let message: string | undefined;
  if (shouldAlert) {
    const prefix = `doctor: ${failCount} failing — `;
    const names = failingChecks.map(c => c.name);
    message = prefix + truncateListForSentryTitle(prefix, names);
  }
