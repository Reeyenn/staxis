...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  // call completes (pg_advisory_xact_lock holds for the implicit
  // transaction). We only protect the prep window, NOT the SSE stream —
  // the stream's atomicity is already covered by recordAssistantTurn's RPC.
  let conversationId = body.conversationId;
  let history: AgentMessage[] = [];
  try {
    if (conversationId) {
      // Acquire conversation lock before loading. If another tab is mid-prep
      // for this conversation, we serialize behind it. Best-effort: if the
      // RPC fails, we proceed without the lock (no worse than today).
      try {
      ...
      try {
        ...
        for await (const event of iter) {
          ...
          if (event.type === 'assistant_turn') {
            ...
            // finally block finalizes (charges the cap) instead of canceling.
            finalUsage = event.usage;
          }
        }
