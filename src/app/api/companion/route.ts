...
export async function POST(req: NextRequest): Promise<Response> {
  let body: {
    ...
    ending?: unknown;
    /**
     * `whats_new_seen` ONLY, and read as a YYYY-MM-DD day.
     *
     * The notices half used to send this too, and no longer does: which batch
     * was announced and when the list was opened are both the server's own
     * facts now, derived from this person's notices in this request. What
     * survives here is a different question wearing the same name — which
     * shipped change a person has caught up to — and it is safe to take from
     * the browser because the worst a wrong day can do is re-show or skip a
     * what's-new card for the person who sent it.
     */
    through?: unknown;
  };
  try {
