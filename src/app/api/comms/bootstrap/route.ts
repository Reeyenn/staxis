...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  // Presence is best-effort telemetry, not response data. Schedule the write
  // after the response so a slow mutation cannot hold the Communications
  // shell behind a loader. The caller is active by definition, so include it
  // in this response even if the previous heartbeat has aged out.
  after(() => touchPresence(ctx.pid, ctx.staffId));
