...
export interface PeopleControllerState {
  ...
  teamError: string;
  teamSettled: boolean;
  /** True once a snapshot for the exact selected hotel + viewer is held, even
   *  while a non-clearing refresh is in flight. Guards actions that must never
   *  run against a previous hotel's roster. */
  teamSnapshotCurrent: boolean;
  /** Hotel setup / first-person onboarding marker from the same response. */
  firstPersonOnboarding: FirstPersonOnboardingSnapshot | null;
...
function parseFirstPersonOnboarding(value: unknown): FirstPersonOnboardingSnapshot | null {
  if (!isRecord(value)) return null;
  const { status, invitedEmail, accountId } = value;
  if (status !== 'none' && status !== 'pending' && status !== 'created') return null;
  if (!isStringOrNull(invitedEmail) || !isStringOrNull(accountId)) return null;
  return { status, invitedEmail, accountId };
}
...
export function usePeopleController(input: PeopleControllerInput): PeopleControllerState {
  ...
  const loadTeam = useCallback(async (
    ...
    try {
      ...
      const responseOnboarding = parseFirstPersonOnboarding(body.data?.firstPersonOnboarding);
      const firstPersonOnboarding = currentInput.adminPreview && !responseOnboarding
        ? null
        : responseOnboarding ?? { status: 'none', invitedEmail: null, accountId: null };
      setSnapshot({
        key: requestKey,
        team: nextTeam,
  ...
  return useMemo(() => ({
    ...
    teamSnapshotCurrent: Boolean(matchingSnapshot),
    firstPersonOnboarding: matchingSnapshot?.firstPersonOnboarding ?? null,
