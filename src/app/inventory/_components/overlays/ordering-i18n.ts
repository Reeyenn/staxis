...
// is told what this thing will and will not do with their money, and it is
// shown exactly once. Every claim in it is one the code actually keeps:
//
//   "we never spend on our own"        — nothing here has a payment method.
//   "we never store card details"      — there is no card field anywhere.
//   "we never touch your email account"— sends go from Staxis's own domain
//                                        with reply-to set to the manager; no
//                                        mailbox credential is ever asked for.
//   "for websites, YOU place it"       — the website button says "Prep my
//                                        order" and links out. There is no
//                                        checkout path, deliberately.
//
...
export function orderingStrings(lang: Lang) {
  return {
    en: {
      ...
      welcomeLine2: 'It never spends money on its own, never stores card details, and never touches your email account.',
      welcomeLine3: 'What it can do depends on how you order:',
      welcomeEmail: 'Email suppliers — Staxis sends the order from our address on your behalf, with your reply address on it. You get a copy, and you get 60 seconds to stop it.',
      ...
      emptyBody: 'This runs off your inventory and your suppliers. Add a few items with par levels and tell us who supplies them, and this screen fills itself in.',
      ...
      basisThin: 'Not enough history yet to work out how fast things move, so this list is based on par levels alone.',
    ...
    es: {
      ...
      welcomeLine2: 'Nunca gasta dinero por su cuenta, nunca guarda datos de tarjetas y nunca entra en tu correo.',
      welcomeLine3: 'Lo que puede hacer depende de cómo pidas:',
      welcomeEmail: 'Proveedores por correo — Staxis envía el pedido desde nuestra dirección de tu parte, con tu dirección para responder. Recibes una copia y tienes 60 segundos para detenerlo.',
      ...
      emptyBody: 'Esto funciona con tu inventario y tus proveedores. Agrega artículos con nivel par y dinos quién los surte, y esta pantalla se llena sola.',
      ...
      basisThin: 'Todavía no hay suficiente historial para saber qué tan rápido se gastan las cosas, así que esta lista se basa solo en los niveles par.',
