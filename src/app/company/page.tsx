...
function CompanyAccessContent() {
  ...
  React.useEffect(() => {
    if (!user || !currentViewerKey || authLoading || propertyLoading || !authorizationChecked) return;
  ...
  }, [authLoading, authorizationChecked, currentViewerKey, propertyLoading, retryKey]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  React.useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [accountId, authLoading, authorizationChecked, authorizationFingerprint, currentData, propertyLoading, retryKey, user, userRole]);
