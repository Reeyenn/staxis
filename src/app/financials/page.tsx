...
import { Notice, T, FONT_SANS, FONT_MONO } from './_components/fin-ui';
...
export default function FinancialsPage() {
  ...
  if (authLoading || propLoading || !accessContextReady) {
    return (
      <AppLayout>
    ...
    <AppLayout>
      <div style={{ background: 'transparent', minHeight: 'calc(100dvh - 64px)' }}>
        ...
        <div style={{ marginTop: 22 }}>
          {tab === 'checkbook' && <CheckbookTab key={financialScopeKey} scopeKey={financialScopeKey} pid={activePropertyId} lang={lang as Lang} month={month} onChanged={onTabChanged} />}
          {tab === 'budget' && <BudgetTab key={financialScopeKey} scopeKey={financialScopeKey} pid={activePropertyId} lang={lang as Lang} month={month} onChanged={onTabChanged} />}
          {tab === 'capex' && <CapexTab key={financialScopeKey} scopeKey={financialScopeKey} pid={activePropertyId} lang={lang as Lang} onChanged={onTabChanged} />}
        </div>
        </div>
      </div>
    </AppLayout>
