...
.mho-tab[aria-selected=true]{background:#3E5C48;color:#fff;border-color:#3E5C48;}
...
export function MultiHotelOpsPanel({
  ...
  const detail = useApiResource<{ replies: LogReplyDTO[] }>(detailUrl, {
  ...
  return (
    <section className="mho" aria-label="Multi-hotel Staxis views">
      ...
      <div className="mho-tabs" role="tablist" aria-label="Hotel operations">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            type="button"
      ...
      {!resource.loading && !resource.error && payload?.coverage && !payload.coverage.complete && (
        <div className="mho-coverage" role="status">
          Complete data was read from {payload.coverage.processedHotelCount} of {payload.coverage.authorizedHotelCount} hotels.
          ...
          {' '}Do not read this as whole-hotel coverage.
        </div>
      )}
    ...
    <ul className="mho-list">
      {entries.map((entry) => (
        <li className="mho-row" key={`${entry.propertyId}:${entry.id}`}>
          <div className="mho-row-top"><span className="mho-hotel">{entry.hotelName}</span><span className="mho-time">{dateLabel(entry.createdAt)}</span></div>
          ...
          {selectedEntry?.id === entry.id && selectedEntry.propertyId === entry.propertyId && (
            <div className="mho-row-body" role="region" aria-label={`Replies for ${entry.title}`}>
              {repliesLoading ? 'Loading replies…' : repliesError ? 'Replies could not be read.' : replies.length === 0 ? 'No replies yet.' : replies.map((reply) => (
    ...
    <ul className="mho-list">
      {items.map((item) => (
        <li className="mho-row" key={`${item.propertyId}:${item.taskId}`}>
          <div className="mho-row-top"><span className="mho-hotel">{item.hotelName}</span><span className="mho-time">{dateLabel(item.createdAt)}</span></div>
