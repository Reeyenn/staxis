...
export function PortfolioQueueView({
  lang,
  organizationId = null,
  authorizationKey: viewerAuthorizationKey,
  onScope,
}: {
  lang: Lang;
  ...
  // A picker change is an authorization-identity change even when the signed-in
  // account and its property grants did not change. Binding both values makes
  // the resource hook synchronously mask the previous company's cards.
  const authorizationKey = `${viewerAuthorizationKey}\u0000${queueUrl}`;
  ...
  const { data, loading, error, reload } = useApiResource<PortfolioPayload>(
    readPortfolio,
    {
      ...
      // A failed receipt reassertion must not leave the prior authorized
      // company rendered as current data. The bounded reader supplies the
      // terminal error/retry state, so the hook needs no second deadline.
