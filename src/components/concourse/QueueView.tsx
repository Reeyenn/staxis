...
export function QueueView({ lang }: { lang: 'en' | 'es' }) {
  ...
  const { data: coverage, error: coverageError } = useApiResource<CoveragePayload>(
    ...
    {
      enabled: !!viewerAuthorizationKey && !!requestedPid,
  ...
  const companyScope = portfolioScopeForViewer(
    ...
    viewerAuthorizationKey,
  );
  const setCurrentCompanyScope = React.useCallback((scope: PortfolioScope | null) => {
    if (!viewerAuthorizationKey) return;
    setCompanyScopeSnapshot({ viewerKey: viewerAuthorizationKey, scope });
  }, [viewerAuthorizationKey]);
  ...
  // navigation.
  if (shouldRenderQueueIdentityLoading(!!user, viewerAuthorizationKey)) {
    return (
  ...
  return (
    <>
      {!!viewerAuthorizationKey && showPortfolioProbe && (
        <PortfolioQueueView
          key={viewerAuthorizationKey}
          lang={lang}
