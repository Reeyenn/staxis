...
registerTool<{ roomNumber: string; staffName: string }>({
  ...
  handler: async ({ roomNumber, staffName }, ctx): Promise<ToolResult> => {
    ...
    // after lookups validated both room + staff exist.
    if (ctx.dryRun) {
      return {
        ok: true,
...
registerTool<{ staffName: string; message: string }>({
  ...
  handler: async ({ staffName, message }, ctx): Promise<ToolResult> => {
    ...
    // Codex post-merge review 2026-05-13 (F2): dryRun skips the SMS-outbox
    // insert (real Twilio costs $) after lookups validated.
    if (ctx.dryRun) {
      return {
        ok: true,
