...
export async function runOneEval(
  ...
  // attributes the spend so the global cap can throttle a runaway CI loop.
  // recordNonRequestCost throws on FK violation (Codex N9 fix) so any
  // schema drift surfaces loudly rather than silently dropping rows.
  try {
