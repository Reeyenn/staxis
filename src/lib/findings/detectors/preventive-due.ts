...
function neverDoneDraft(task: PreventiveScheduleEntry): FindingDraft {
  ...
  return {
    ...
    summary:
      `${task.name}${where} is on this hotel's upkeep schedule — every ${cadence} — but has never `
    ...
    evidence: {
      ...
      basis:
        `${task.name} has a cadence of every ${cadence} and no recorded completion, so there is no `
