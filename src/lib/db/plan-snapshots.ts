...
// CsvRoomSnapshot stays in db/schedule-assignments.ts (the original
// owner). The Schedule tab uses the simpler { number, type } shape
// from there, not the full PlanSnapshot.rooms[N] shape.

/**
 * CSV pipeline status — was tied to Railway scraper. Kept as a type
 * so callers compile, but values are always null post-v4.
 */
...
export interface CsvFreshnessResult {
  state: CsvFreshness;
  /** "Today 6:04 AM" style label; null when nothing has been pulled yet. */
  label: string | null;
  /** Truthy when state==='error' so the UI can colour-code. */
  errorCode?: string | null;
}
...
/**
 * The old "is the CSV pull fresh?" check. Post-v4 the CUA writes
 * continuously to pms_room_status_log so a fresh latest event ≈ fresh
 * data. We classify based on the most-recent pms_room_status_log row.
 *
 * Returns 'fresh' if any event landed in the last `staleMinutes` (default
 * 75). 'stale' if older. The Schedule tab uses this to surface the
 * "Data is X minutes old" badge.
 */
export function csvFreshness(
  latestEventAt: Date | null,
  opts: CsvFreshnessOptions = {},
): CsvFreshnessResult {
  const now = opts.now ?? new Date();
  const staleMin = opts.staleMinutes ?? CSV_STALE_MINUTES;
  if (!latestEventAt) {
    return { state: 'stale', label: null };
  }
  const ageMin = (now.getTime() - latestEventAt.getTime()) / 60_000;
  const label = formatRecency(latestEventAt, now);
  if (ageMin > staleMin) return { state: 'stale', label };
  return { state: 'fresh', label };
}

function formatRecency(at: Date, now: Date): string {
  const sameDay = at.toLocaleDateString('en-CA') === now.toLocaleDateString('en-CA');
  const time = at.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' });
  return sameDay ? `Today ${time}` : `${at.toLocaleDateString('en-US', { weekday: 'short' })} ${time}`;
}

/**
 * (Plan v4) Always returns nulls — the CSV pipeline is gone.
 * Kept so callers compile.
 */
export function subscribeToCsvPipelineStatus(
  _pid: string,
  callback: (status: CsvPipelineStatus) => void,
): () => void {
  callback({ morning: null, evening: null });
  return () => {};
}
