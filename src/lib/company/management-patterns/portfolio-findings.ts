...
function requireSourcePackageConsistency(source: SourcePackage, input: {
  ...
  if (source.run !== null) {
    ...
    if (
      ...
      || coverage.exclusion_reason_code_count < coverage.exclusion_reasons.length
...
export async function loadManagementPatternPortfolioFindings(
  ...
  const parsedInput = z.object({
    accountId: uuid,
    scopeReceiptId: uuid,
    selectedPropertyIds: authorizationPropertyIds,
  ...
  if (selectedPropertyIds.length > 250) {
    return emptyLoadReceipt({
      accountId: parsedInput.accountId,
      scopeReceiptId: parsedInput.scopeReceiptId,
      selectedPropertyIds,
      ...
      receipt: before.receipt,
      reason: 'consumer_scope_limit_exceeded',
    });
  }
