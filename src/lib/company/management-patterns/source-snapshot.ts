...
const activityStreamBaseShape = {
  ...
  // V1 has no immutable, deletion-resistant activity ledger. Keep this a
...
const sourcePropertySchema = z.object({
  ...
  const supplyFreshThrough = maximumInstant(
  ...
  if (
    ...
    || property.supply.fresh_through !== supplyFreshThrough
    || property.supply.source_watermark.max_closed_at !== supplyFreshThrough
  ...
  if (
    ...
    || property.rooms_sold.fresh_through !== roomsFreshThrough
    || property.rooms_sold.source_watermark.max_updated_at !== roomsFreshThrough
...
export async function loadManagementPatternSourceSnapshot(input: {
  ...
  const window = completedSupplyWindow(input.evaluationAt);
