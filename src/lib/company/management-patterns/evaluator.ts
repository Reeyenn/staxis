...
function preparedContractReasons(prepared: PreparedManagementPatternInputs): readonly string[] {
  ...
  const expectedWindow = expectedSupplyWindow(snapshot.evaluation_at);
  if (
    expectedWindow === null
...
function cohortCandidates(prepared: PreparedManagementPatternInputs): readonly CohortCandidate[] {
  return Object.freeze(prepared.properties.flatMap((property) => {
    if (property.profile === null) return [];
    const reasons = supplyReasons(property);
    return [{
      profile: property.profile,
      eligibility: reasons.length === 0 && normalizedSupply(property) !== null
...
export function supplyProfileCoverageReceipt(
  ...
  const completeRatio = population.length === 0
    ? 0
    : completeProfileCount / population.length;
  const payload = {
    status: completeRatio >= SUPPLY_SPEND_PROFILE_COVERAGE_POLICY.minimumCompleteRatio
      ? 'passed' as const
      : 'abstained' as const,
    ...
    populationPropertyCount: population.length,
...
export function replaySupplyCohortDecision(
  ...
  if (
    ...
    || supplyProfileCoverageReceipt(prepared, targetPropertyId).status !== 'passed'
  ) return null;
  const normalized = normalizedSupply(target);
  if (
    normalized === null
    || supplySpendMaterialityThreshold(
      normalized.currency,
      normalized.currencyStorageScale,
    ) === null
  ) return null;
...
function evaluateSupply(
  ...
  for (const target of [...prepared.properties].sort((left, right) => propertyId(left).localeCompare(propertyId(right)))) {
    ...
    const profileCoverage = supplyProfileCoverageReceipt(prepared, targetId);
    if (profileCoverage.status === 'abstained') {
      outcomes.push(outcome({
        ...
        evidence: canonicalize({ profileCoverage }),
...
export function evaluateManagementPatterns(
  ...
  const payload = {
    schemaVersion: MANAGEMENT_PATTERN_EVALUATOR_VERSION,
    organizationId: prepared.snapshot.organization.id,
    evaluatedAt: prepared.snapshot.evaluation_at,
    inputFingerprint: prepared.fingerprint,
