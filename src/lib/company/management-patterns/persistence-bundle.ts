...
import {
  compactSupplyProfileCoverageReceipt,
  replaySupplyCohortDecision,
    ...
    cohorts.push(Object.freeze({
      ...
      definition: {
        ...
        coverage_policy: {
          ...
          population_basis: 'target_rung_potentially_compatible_leave_one_out_peers',
        },
...
function replayedCohortDecisions(
  ...
  for (const outcome of outcomes) {
    ...
    if (outcome.reasonCodes.includes('insufficient_profile_dimension_coverage')) {
      const evidence = objectValue(outcome.evidence);
      const declaredProfileCoverage = evidence === null
        ? null
        : objectValue(evidence.profileCoverage);
      const expectedProfileCoverage = compactSupplyProfileCoverageReceipt(
        supplyProfileCoverageReceipt(prepared, outcome.targetPropertyId),
      );
      if (
        declaredProfileCoverage === null
        || JSON.stringify(declaredProfileCoverage) !== JSON.stringify(expectedProfileCoverage)
      ) {
        throw new TypeError(
          `supply outcome ${outcome.outcomeKey} profile coverage replay receipt mismatch`,
        );
      }
    }
