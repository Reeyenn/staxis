...
export interface PortfolioToolCounts {
  countsByPropertyId: Map<string, number>;
  /** Requested ids omitted by the DB's live company/hotel intersection. */
  unavailablePropertyIds: string[];
}

export interface PortfolioToolWorkOrderSummary {
  propertyId: string;
  opened: number;
  stillOpen: number;
  stillOpenBySeverity: {
  ...
  recordedRepairSpend: number | null;
  repairCostSamples: number;
}

export interface PortfolioToolWorkOrderSummaries {
  summariesByPropertyId: Map<string, PortfolioToolWorkOrderSummary>;
  /** Requested ids omitted by the DB's live company/hotel intersection. */
  unavailablePropertyIds: string[];
}

export interface PortfolioFindingCount {
  propertyId: string;
  openFindings: number;
  needsDecision: number;
}

export interface PortfolioToolHotelRow {
  id: string;
  name: string | null;
...
export async function readPortfolioToolHotels(
  organizationId: string,
  propertyIds: readonly string[],
): Promise<{ hotels: PortfolioToolHotelRow[]; unavailablePropertyIds: string[] }> {
  const { ids, allowed } = boundedScope(organizationId, propertyIds);
  if (ids.length === 0) return { hotels: [], unavailablePropertyIds: [] };

  const { data, error } = await supabaseAdmin.rpc('staxis_portfolio_tool_hotels', {
    p_organization_id: organizationId,
    p_property_ids: ids,
  }) as unknown as RpcResult;
  if (error) {
    throw new Error(`staxis_portfolio_tool_hotels failed: ${error.message ?? 'unknown database error'}`);
  }

  const seen = new Set<string>();
  const hotels = rawObjects(data, 'staxis_portfolio_tool_hotels').map((row) => {
    const id = typeof row.property_id === 'string' ? row.property_id : null;
    if (!id || !allowed.has(id) || seen.has(id)) {
      throw new Error('staxis_portfolio_tool_hotels crossed or duplicated property scope');
    }
    seen.add(id);
    const rooms = row.total_rooms === null || row.total_rooms === undefined
      ? null
      : Number(row.total_rooms);
    if (rooms !== null && !Number.isFinite(rooms)) {
      throw new Error(`staxis_portfolio_tool_hotels returned invalid room count for ${id}`);
    }
    return {
      id,
  ...
  return {
    hotels,
    unavailablePropertyIds: ids.filter((id) => !seen.has(id)),
  };
}
...
export async function readPortfolioToolFindingCounts(
  organizationId: string,
  propertyIds: readonly string[],
  statuses: readonly string[],
): Promise<{
  counts: PortfolioFindingCount[];
  unavailablePropertyIds: string[];
}> {
  const { ids, allowed } = boundedScope(organizationId, propertyIds);
  if (statuses.length === 0 || statuses.length > 6) {
    throw new Error('portfolio finding count status set must contain 1..6 values');
  }
  if (ids.length === 0) return { counts: [], unavailablePropertyIds: [] };
  ...
  if (error) {
    throw new Error(
      `staxis_portfolio_tool_finding_counts failed: ${error.message ?? 'unknown database error'}`,
    );
  }
  const seen = new Set<string>();
  const counts = rawObjects(data, 'staxis_portfolio_tool_finding_counts').map((row) => {
    const propertyId = typeof row.property_id === 'string' ? row.property_id : null;
    const openFindings = Number(row.open_count);
    const needsDecision = Number(row.needs_decision_count);
    if (!propertyId || !allowed.has(propertyId) || seen.has(propertyId)) {
      throw new Error('staxis_portfolio_tool_finding_counts crossed or duplicated property scope');
    }
    if (!Number.isSafeInteger(openFindings)
      || openFindings < 0
      || !Number.isSafeInteger(needsDecision)
      || needsDecision < 0
      || needsDecision > openFindings) {
      throw new Error(`staxis_portfolio_tool_finding_counts returned invalid counts for ${propertyId}`);
    }
    seen.add(propertyId);
    return { propertyId, openFindings, needsDecision };
  });
  return {
    counts,
    unavailablePropertyIds: ids.filter((id) => !seen.has(id)),
...
export async function readPortfolioToolWorkOrders(
  organizationId: string,
  propertyIds: readonly string[],
  financialPropertyIds: readonly string[],
  since: string,
): Promise<PortfolioToolWorkOrderSummaries> {
  const { ids, allowed } = boundedScope(organizationId, propertyIds);
  const requested = new Set(ids);
  if (financialPropertyIds.some((id) => !requested.has(id))) {
    throw new Error('portfolio work-order financial scope exceeds requested properties');
  }
  if (ids.length === 0) {
    return { summariesByPropertyId: new Map(), unavailablePropertyIds: [] };
  }
  const { data, error } = await supabaseAdmin.rpc('staxis_portfolio_tool_work_orders', {
    p_organization_id: organizationId,
    p_property_ids: ids,
    p_financial_property_ids: [...new Set(financialPropertyIds)],
    p_since: since,
  }) as unknown as RpcResult;
  if (error) {
    throw new Error(
      `staxis_portfolio_tool_work_orders failed: ${error.message ?? 'unknown database error'}`,
    );
  }

  const summariesByPropertyId = new Map<string, PortfolioToolWorkOrderSummary>();
  const count = (value: unknown, field: string, propertyId: string): number => {
    const parsed = Number(value);
    if (!Number.isSafeInteger(parsed) || parsed < 0) {
      throw new Error(`staxis_portfolio_tool_work_orders returned invalid ${field} for ${propertyId}`);
    }
    return parsed;
  };
  for (const row of rawObjects(data, 'staxis_portfolio_tool_work_orders')) {
    const propertyId = typeof row.property_id === 'string' ? row.property_id : null;
    if (!propertyId || !allowed.has(propertyId) || summariesByPropertyId.has(propertyId)) {
      throw new Error('staxis_portfolio_tool_work_orders crossed or duplicated property scope');
    }
    const opened = count(row.opened_count, 'opened count', propertyId);
    const stillOpen = count(row.still_open_count, 'open count', propertyId);
    const urgent = count(row.urgent_open_count, 'urgent count', propertyId);
    const high = count(row.high_open_count, 'high count', propertyId);
    const normal = count(row.normal_open_count, 'normal count', propertyId);
    const low = count(row.low_open_count, 'low count', propertyId);
    const ungraded = count(row.ungraded_open_count, 'ungraded count', propertyId);
    const repairCostSamples = count(row.repair_cost_samples, 'repair-cost sample count', propertyId);
    const rawSpend = row.repair_cost_sum;
    const recordedRepairSpend = rawSpend === null ? null : Number(rawSpend);
    if (stillOpen > opened
        || urgent + high + normal + low + ungraded !== stillOpen
        || (recordedRepairSpend !== null
          && (!Number.isFinite(recordedRepairSpend) || recordedRepairSpend < 0))
        || (repairCostSamples === 0) !== (recordedRepairSpend === null)) {
      throw new Error(`staxis_portfolio_tool_work_orders returned inconsistent totals for ${propertyId}`);
    }
    summariesByPropertyId.set(propertyId, {
      propertyId,
      opened,
      stillOpen,
      stillOpenBySeverity: { urgent, high, normal, low, ungraded },
      recordedRepairSpend,
      repairCostSamples,
    });
  }
  return {
    summariesByPropertyId,
    unavailablePropertyIds: ids.filter((id) => !summariesByPropertyId.has(id)),
  };
}

export function readPortfolioToolInventoryOrders(
  organizationId: string,
  propertyIds: readonly string[],
  since: string,
  limitPerProperty: number,
): Promise<PortfolioToolRows> {
  return rowsRpc('staxis_portfolio_tool_inventory_orders', organizationId, propertyIds, {
    p_since: since,
    p_limit_per_property: boundedRowLimit(limitPerProperty),
  });
}

export function readPortfolioToolInventory(
  organizationId: string,
  propertyIds: readonly string[],
  limitPerProperty: number,
): Promise<PortfolioToolRows> {
  return rowsRpc('staxis_portfolio_tool_inventory', organizationId, propertyIds, {
    p_limit_per_property: boundedRowLimit(limitPerProperty),
  });
}

export async function readPortfolioToolWorkOrderCounts(
  organizationId: string,
  propertyIds: readonly string[],
  since: string,
): Promise<PortfolioToolCounts> {
  const { ids, allowed } = boundedScope(organizationId, propertyIds);
  if (ids.length === 0) {
    return { countsByPropertyId: new Map(), unavailablePropertyIds: [] };
  }

  const { data, error } = await supabaseAdmin.rpc('staxis_portfolio_tool_work_order_counts', {
    p_organization_id: organizationId,
    p_property_ids: ids,
    p_since: since,
  }) as unknown as RpcResult;
  if (error) {
    throw new Error(
      `staxis_portfolio_tool_work_order_counts failed: ${error.message ?? 'unknown database error'}`,
    );
  }

  const countsByPropertyId = new Map<string, number>();
  for (const row of rawObjects(data, 'staxis_portfolio_tool_work_order_counts')) {
    const propertyId = typeof row.property_id === 'string' ? row.property_id : null;
    const count = Number(row.metric_count);
    if (!propertyId || !allowed.has(propertyId) || countsByPropertyId.has(propertyId)) {
      throw new Error('staxis_portfolio_tool_work_order_counts crossed or duplicated property scope');
    }
    if (!Number.isSafeInteger(count) || count < 0) {
      throw new Error(`staxis_portfolio_tool_work_order_counts returned invalid count for ${propertyId}`);
    }
    countsByPropertyId.set(propertyId, count);
  }
  return {
    countsByPropertyId,
    unavailablePropertyIds: ids.filter((id) => !countsByPropertyId.has(id)),
  };
}
