...
async function readCompanyForProperty(propertyId: string): Promise<string | null> {
  ...
  if (real.length > 1) {
    // NO COMPANY, LOUDLY.
    //
    // This function decides whose rulebook goes into the hotel's prompt, whose
    // money thresholds apply to it, and which company's people appear on its
    // team list. It used to answer `real[0]` — the lowest UUID — which is not a
    ...
    // which is a data problem a human has to settle. Until they do, the hotel
    // behaves as an independent one: no rulebook, no company thresholds,
    // nothing borrowed from a company that may not be the right one.
