...
import { billedBundleKeys, employeeSpend, spendAttributionNote } from '@/lib/ai/employee-spend';
...
interface RosterEmployee {
  id: string;
  spend: { known: boolean; usd: number | null; untracked: string[] } | null;
}
...
describe('agent_costs.feature — the ledger learns which job spent the money', () => {
  ...
  test('an employee with several features is billed for those, not for the whole read', () => {
    // One read serves the whole roster, so the map legitimately carries keys
    // belonging to other employees.
    const totals = new Map<string, number>([
      ['findings.brief', 0.20],
      ['findings.judge', 0.05],
      ['agent.ask_staxis', 9.00],
      ['inventory.invoice_scan', 4.00],
    ]);
  ...
  test('a feature with no rows in the window is zero, not missing', () => {
    const spend = employeeSpend(synthetic(['findings.brief', 'findings.sweep']), new Map([['findings.brief', 0.20]]));
  ...
  test('an employee whose work never produces a bill reports no figure, not $0.00', () => {
    ...
    // costs hosting; it simply is not billed per call.
    const spend = employeeSpend(synthetic(['ml.housekeeping_optimizer']), new Map());
