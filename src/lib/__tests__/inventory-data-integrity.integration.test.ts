/**
 * Executable database regression coverage for migration 0310.
 *
...
describe('inventory migration 0310 executable integrity', () => {
  before(async () => {
    const migrated = await applyMigrationsToPglite();
    pg = migrated.pg;
    assert.ok(
      migrated.report.applied.includes('0310_inventory_data_integrity.sql'),
      `0310 must apply in PGlite: ${JSON.stringify(migrated.report.failedAtRuntime.filter((x) => x.file.startsWith('0310')))}`,
    );
