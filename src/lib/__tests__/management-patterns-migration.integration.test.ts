...
function exactSupplySourceFacts(observationId = OBSERVATION) {
  const numerator = ['2026-01-01', '2026-02-01', '2026-03-01'].map((month, index) => {
    const nextMonth = ['2026-02-01', '2026-03-01', '2026-04-01'][index]!;
    const id = `71000000-0000-4000-8000-00000000000${index + 1}`;
    const updatedAt = `${nextMonth}T06:00:00Z`;
    const factPayload = {
      id,
      month_start: month,
      ...
      source_query_version: 'management-pattern-source-snapshot.v1',
...
describe('management-company patterns migration 0390', () => {
  before(async () => {
    ...
    await pg.exec(`
      ...
      -- Migration 0376 owns this locking assertion in production. This narrow
      -- fixture preserves its public contract so 0390 can prove that it never
      -- trusts caller-supplied organization/property IDs.
  ...
  test('keeps comparison profile facts nullable and versions immutable', async () => {
    const profile = await one<{ id: string; currency_code: string | null }>(
    ...
    await assert.rejects(
      pg.query(
        `insert into public.management_pattern_property_profiles (
         ...
         ) values ($1, $2, $3, 2, '2026-08-01', 'usd', 2,
  ...
  test('claims/resumes with a lease and rejects a live competing owner', async () => {
    ...
    const expiredRun = await one<{ id: string }>(
      `insert into public.management_pattern_runs (
         ...
         scope_policy_version, input_hash, portfolio_snapshot, portfolio_snapshot_hash,
  ...
  test('accepts one exact atomic result batch and rejects a changed retry', async () => {
  ...
  test('batch-ingests exact tenant-paired raw and normalized observations', async () => {
        ...
        JSON.stringify([
          {
            ...
            denominator_freshness_age_seconds: 1800,
            denominator_source_query_id: 'management_pattern_daily_log_occupancy',
            denominator_source_query_version: 'management-pattern-source-snapshot.v1',
            ...
            denominator_source_watermark: {
              fresh_through: '2026-04-01T12:00:00Z',
              source_revision: 'denominator-revision',
              receipt: sourceAccessWatermark({ max_updated_at: '2026-04-01T12:00:00Z' }),
            },
            ...
            source_watermark: {
              fresh_through: '2026-04-01T06:00:00Z',
              source_revision: 'numerator-revision',
              receipt: sourceAccessWatermark({ max_closed_at: '2026-04-01T06:00:00Z' }),
            },
    ...
    partialObservation.quality_status = 'partial';
    partialObservation.quality_reasons = ['numerator_missing', 'occupancy_partial'];
  ...
  test('keeps v1 shadow-only and retains immutable evidence without live relationship FKs', async () => {
    await assert.rejects(
      pg.query(
        `select * from public.project_management_pattern_candidate($1, $2)`,
        [ORG_A, CANDIDATES[0]],
