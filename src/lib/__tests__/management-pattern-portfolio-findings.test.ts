...
const LARGE_AUTHORIZED_SCOPE = Object.freeze(Array.from({ length: 252 }, (_, index) => (
...
function sourceCandidate(overrides: Record<string, unknown> = {}) {
  return {
    ...
    source_query_receipts: [
      { id: 'inventory_periods.aggregate', versions: ['management-pattern-source-snapshot.v1'] },
      { id: 'rooms_sold.daily', versions: ['management-pattern-source-snapshot.v1'] },
    ],
    ...overrides,
  };
}
...
function sourcePackage(overrides: Record<string, unknown> = {}) {
  return {
    ...
    run: {
      id: RUN,
      projection_mode: 'active',
      engine_version: 'management-pattern-engine.v1',
...
test('portfolio producer preserves the full authorization universe separately from one selected hotel', async () => {
  ...
  assert.equal(result.coverage.authorizedPropertyCount, 252);
...
test('portfolio producer rejects 251 selected hotels before querying without shrinking authorization', async () => {
  ...
  const result = await loadManagementPatternPortfolioFindings({
    ...
    assertAuthorizationScopeReceipt: mockAssertion([receipt]),
    client: mockClient(sourcePackage(), () => { queried = true; }),
  });

  assert.equal(queried, false);
  assert.equal(result.status, 'scope_too_large');
  assert.equal(result.coverage.authorizedPropertyCount, 252);
  assert.equal(result.coverage.selectedPropertyCount, 251);
  assert.deepEqual(result.findings, []);
});
...
test('portfolio producer keeps shadow and changed-run scopes out of claims', async () => {
  ...
  assert.equal(shadow.run?.engineVersion, 'management-pattern-engine.v1');
...
test('portfolio producer never revives stale or abstained evidence', async () => {
  ...
  assert.equal(staleResult.run?.engineVersion, 'management-pattern-engine.v1');
