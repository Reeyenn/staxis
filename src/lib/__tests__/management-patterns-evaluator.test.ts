...
function sourceProperty(index: number, options: PropertyOptions = {}): ManagementPatternSourceProperty {
  ...
  // immutable-ledger source. Production v1 parsing intentionally rejects these
  ...
  return {
    ...
    profile: {
      ...
      effective_to: null,
      source_kind: 'property_authoritative',
      source_reference: `fixture:${index}`,
      created_at: '2025-01-01T00:00:00.000Z',
      room_count: options.incompleteProfile ? null : 100,
      timezone: 'UTC',
      business_date_cutoff_hour: 0,
      service_level: options.incompleteProfile ? null : 'full_service',
      market_type: 'urban',
      brand_class: 'upper_upscale',
      location_type: 'urban',
      operating_model: 'managed',
      amenity_tags: [],
      currency_code: options.incompleteProfile ? null : currency,
      currency_minor_unit_exponent: options.incompleteProfile ? null : 2,
    ...
    supply: {
      ...
      fresh_through: '2026-07-01T00:00:00.000Z',
      source_watermark: {
    ...
    rooms_sold: {
      ...
      fresh_through: options.missingDenominator ? null : '2026-07-01T00:00:00.000Z',
      source_watermark: {
  ...
  test('does not let six complete profiles masquerade as full coverage in a 50-hotel portfolio', () => {
    ...
    const evidence = target?.evidence as {
      profileCoverage?: {
        ...
        exclusions: Array<{ propertyId: string; reasonCodes: string[] }>;
        fingerprint: string;
      };
    } | undefined;
    ...
    assert.equal(receipt?.exclusions[0].propertyId, uuid(7));
    assert.ok(receipt?.exclusions[0].reasonCodes.includes(
    ...
    assert.equal(
      ...
      'supply-spend-profile-domain-coverage.v1',
    );
...
describe('management pattern persistence graph', () => {
  test('maps a real multi-outcome evaluation with exact root and receipt lineage', () => {
    ...
    assert.equal(coveragePolicy?.population_basis, 'dimension_compatible_leave_one_out_peers');
