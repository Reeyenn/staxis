...
describe('management pattern source snapshot migration 0387', () => {
  ...
  test('late mutable source revisions are excluded rather than backfilled', async () => {
    await pg.exec(`
      update public.daily_logs
      set updated_at = '2026-04-06T00:00:00Z'
      where property_id = '${PROPERTY_A}' and date = '2026-02-01';
    `);
    const result = await snapshot();
    const property = result.properties[0]!;
    assert.equal(property.rooms_sold.observed_days, 89);
    assert.equal(property.rooms_sold.normalization_eligible, false);
    assert.equal(property.rooms_sold.days.some((day) => day.date === '2026-02-01'), false);
  });
