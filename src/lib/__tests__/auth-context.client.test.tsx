...
type RealtimeChannelSurface = {
  on: (...args: unknown[]) => RealtimeChannelSurface;
  subscribe: () => RealtimeChannelSurface;
};

type RealtimeSurface = {
  channel: (name: string) => RealtimeChannelSurface;
  removeChannel: (channel: RealtimeChannelSurface) => Promise<unknown>;
};
...
async function mountProvider(context: TestContext, fakeTimers = false): Promise<{
  ...
  context.mock.method(realtime, 'channel', () => {
    const channel: RealtimeChannelSurface = {
      on: () => channel,
      subscribe: () => channel,
    };
    return channel;
  });
  context.mock.method(realtime, 'removeChannel', async () => 'ok');
