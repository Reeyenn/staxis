...
describe('My Hotel account and team integration', () => {
  ...
  test('includes member editing, removal, staff approvals, and both invitation paths', () => {
    ...
    assert.match(hotelTeamDialogs, /What does this person need\?/);
    assert.match(hotelTeamDialogs, /NO LOGIN/);
    assert.match(hotelTeamDialogs, /Add to schedule only/);
    assert.match(hotelTeamDialogs, /Add them to the hotel roster and schedule\. They cannot sign in to Staxis\./);
    ...
    assert.match(hotelTeam, /Use Invite people to add someone to the schedule only, or invite them to create a Staxis login/);
    assert.match(hotelTeamDialogs, /Hotel invite/);
    assert.match(hotelTeamDialogs, /Email one person/);
