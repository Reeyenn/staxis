...
function ctx(): ToolHandlerContext {
  return {
    ...
    requestId: 'req-test',
    surface: 'chat',
    // The tools these tests exercise read through the scoped accessor, not a
    // raw client — so the fake must hand them one, or the handler has nothing
    // to query. scopedDb wraps the same stubbed supabaseAdmin.from below, so
    // the `touched` assertions still see every table name.
    db: scopedDb(PID),
  };
}
