...
rows for a property the caller lacks access to. The only family-scoped
exceptions are `pms_knowledge_files`, `pms_table_schemas`,
`pms_writeback_recipes` — shared across every hotel on a PMS family by design.
