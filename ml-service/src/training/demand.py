...
def _train_demand_inner(
    ...
    # Codex adversarial review 2026-05-13 (A6): use the atomic RPC
    # (migration 0107) instead of a raw insert. The RPC wraps deactivate
    # + insert in one transaction with a per-(property, layer) advisory
    # lock, so two concurrent trainings can't both insert is_active=true
    # and fight the partial unique index `model_runs_active_housekeeping_uq`.
    fields = {
    ...
    if not row.get("ok"):
        # The RPC refused for a known reason (e.g. invalid_layer) — surface
        # via the standard error contract.
        return {
