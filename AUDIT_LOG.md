...
- **[MED] Single-hotel manager can mutate a multi-hotel staff account** (#12) — password reset / deactivate is a *global* credential change that affects the user at their OTHER hotels too. Partially mitigated by the #1 privilege matrix; the full fix (scope credential changes, or warn) is a product-policy call.
...
- **[LOW] Phone-backfill admin tools unbounded** (#33), **nudge/cost-account global scans** (#34) — perf hardening on admin/rare paths; low real-world impact at current scale.
