...
- **[MED] Single-hotel manager mutating a multi-hotel staff account** (#12) — **PASSWORD RESET now guarded (conservative default, 2026-06-18):** a non-admin manager can no longer reset the password of someone who also works at a hotel the manager doesn't control (password reset is a *global* credential change / account-takeover vector). **Relax this** if you want managers to have broader reach — it's a one-line change, flagged in `/api/auth/team` PUT. The parallel `deactivate` action in `/api/settings/users` (reversible, lower-harm) is the same pattern and left for you to extend the same way if you want.
