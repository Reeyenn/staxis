...
- **[MED] PO receive loses stock on partial DB failure** (#10) — **FIXED & LIVE (2026-06-18, main `98a51491`).** Migration 0286 adds `staxis_receive_po_lines(property, po, lines)`, which applies every line's `qty_received` bump AND its `inventory.current_stock` increment in ONE transaction (all-or-nothing, safely retryable). The stock bump is now a single atomic `current_stock + delta` (no read-modify-write), so the prior "received but stock never moved → permanently understated" failure is gone. Writes stay id-scoped (line by PO, inventory by property); SECURITY INVOKER, EXECUTE to `service_role` only. `inventory_orders` ledger insert + `purchase_orders` status update keep their best-effort/derived semantics. Migration applied to prod + schema reloaded; tsc/eslint/build/2324 tests green.
...
- **[LOW, but security-adjacent] Idempotency keyspace is global across tenants** (#29) — **RE-AUDITED 2026-06-18: NO REAL BUG, no change needed.** The shared `claim_idempotency_key` RPC does key on `(key, route)` with no `property_id` dimension, BUT tracing every actual caller shows no exploitable collision path: (1) its ONLY consumer is `send-shift-confirmations`, whose key is a per-click `crypto.randomUUID()` — globally unique, so two hotels colliding would require independently generating the same random UUID (astronomically impossible); (2) the rules-engine dedup row carries `property_id` AND `dedupe_key` as a composite (`merger.ts:198/201`), so it's already tenant-scoped; (3) SMS `enqueueSms` dedups on `(property_id, idempotency_key)`; (4) Resend email keys embed a `scheduleId`/`runId` UUID per property. Namespacing the keyspace would be tidier defense-in-depth but fixes nothing reachable — deferred as optional, not a live risk.
