...
--   * the projection RPC serializes by (organization, semantic family, root)
--     and refuses an older candidate after a newer projection
...
create table if not exists public.management_pattern_property_profiles (
  ...
  constraint management_pattern_property_profiles_org_property_from_key
...
begin
  if not exists (
    ...
    join public.organization_property_relationships relationship
      on relationship.organization_id = organization_row.id
     and relationship.id = new.property_relationship_id
     and relationship.property_id = new.property_id
    join public.properties property_row on property_row.id = new.property_id
    where organization_row.id = new.organization_id
      ...
      and (
        ...
        or (
          ...
          and new.effective_to <= relationship.ends_at
        )
      )
  ) then
    raise exception 'management pattern profile must be created against one live tenant/property relationship'
      using errcode = '23514';
  end if;
  return new;
end
...
begin
  ...
  if not found
    or v_manifest_property->>'relationship_id'
      <> new.membership_relationship_id::text
    or v_manifest_property->>'property_snapshot_hash'
      <> new.property_snapshot_hash
  ...
  if jsonb_typeof(v_relationship) <> 'object'
    or v_relationship->>'id' <> new.membership_relationship_id::text
    or v_relationship->>'relationship_type' not in ('operator','owner')
    or v_history_proof_kind not in (
...
begin
  ...
  if v_fencing_token <> new.run_fencing_token then
...
begin
  ...
  if new.fact_role = 'numerator' then
    ...
    if new.fact_payload->>'status' not in ('open','closed')
      or (
    ...
    if not (new.fact_payload ?& array[
      'date','rooms_sold','occupancy_source','sealed_at','seal_version',
  ...
  begin
    ...
    if new.fact_role = 'numerator' then
      v_payload_expected_included := coalesce(
        ...
        and (new.fact_payload->>'source_window_compatible')::boolean
  ...
  if v_fact_date is null
    ...
    or new.fact_payload->>'source_query_id' <> new.source_query_id
    or new.fact_payload->>'source_query_version' <> new.source_query_version
...
begin
  ...
  if jsonb_typeof(p_portfolio_snapshot) <> 'object'
    or p_portfolio_snapshot->>'schema_version' <> '1'
    or p_portfolio_snapshot->>'organization_id' <> p_organization_id::text
    or jsonb_typeof(p_portfolio_snapshot->'properties') <> 'array'
    or jsonb_typeof(p_portfolio_snapshot->'source_budget_exceeded') <> 'boolean'
    or coalesce(p_portfolio_snapshot->>'property_count', '') !~ '^(0|[1-9][0-9]*)$'
  then
    raise exception 'portfolio snapshot is not the strict management-pattern v1 manifest'
      using errcode = '22023';
  end if;
  begin
    ...
    if (p_portfolio_snapshot->>'evaluation_at')::timestamptz <> p_evaluation_at
      or (p_portfolio_snapshot->>'source_as_of')::timestamptz <> p_source_as_of
      or (p_portfolio_snapshot->>'topology_as_of')::timestamptz <> p_topology_as_of then
  ...
  if v_source_budget_exceeded is distinct from (v_snapshot_property_count > 50)
    ...
    or exists (
      ...
      where jsonb_typeof(property) <> 'object'
        ...
        or property->>'eligibility_status' not in ('included','excluded')
        or jsonb_typeof(property->'exclusion_codes') <> 'array'
        or exists (
  ...
  if p_portfolio_snapshot->>'organization_type' <> v_organization_type then
  ...
  if v_run.input_hash <> p_input_hash
    or v_run.triggered_by <> p_triggered_by
    or v_run.projection_mode <> p_projection_mode
    or v_run.engine_version <> p_engine_version
    or v_run.evidence_schema_version <> p_evidence_schema_version
    or v_run.cohort_policy_version <> p_cohort_policy_version
    or v_run.normalization_policy_version <> p_normalization_policy_version
    or v_run.dedupe_policy_version <> p_dedupe_policy_version
    or v_run.scope_policy_version <> p_scope_policy_version
    or v_run.model_versions is distinct from p_model_versions
    or v_run.model_call_budget <> p_model_call_budget
    or v_run.token_budget <> p_token_budget
    or v_run.cost_budget_microusd <> p_cost_budget_microusd
    or v_run.duration_budget_ms <> p_duration_budget_ms
    or v_run.db_query_budget <> p_db_query_budget
    or v_run.portfolio_snapshot_hash <> p_portfolio_snapshot_hash
    ...
    or v_run.evaluation_at <> p_evaluation_at
    or v_run.source_as_of <> p_source_as_of
    or v_run.topology_as_of <> p_topology_as_of
    or v_run.window_start <> p_window_start
    or v_run.window_end <> p_window_end
...
begin
  ...
  if v_run.owner_token <> p_owner_token or v_run.fencing_token <> p_fencing_token then
...
begin
  if p_terminal_status not in ('succeeded','abstained','failed') then
  ...
  if p_terminal_status in ('succeeded','abstained') then
    ...
    if not found or v_result_batch.run_fencing_token <> p_fencing_token then
...
begin
  if jsonb_typeof(p_run_properties) <> 'array'
    or jsonb_typeof(p_metric_observations) <> 'array'
    or jsonb_typeof(p_metric_source_facts) <> 'array' then
  ...
  if jsonb_array_length(p_run_properties) > 5000
    ...
    or pg_column_size(p_metric_source_facts) > 16777216 then
  ...
  if v_run.status not in ('claimed','running')
    or v_run.owner_token <> p_owner_token
    or v_run.fencing_token <> p_fencing_token
...
begin
  ...
  select x.id, p_organization_id, p_run_id, p_fencing_token, x.check_outcome_id,
    ...
    coalesce(x.candidate_schema_version, 1)
  from jsonb_populate_recordset(
...
begin
  ...
  raise exception 'active management-pattern projection is disabled in schema v1'
    using errcode = '0A000';
...
comment on function public.project_management_pattern_candidate(uuid,uuid) is
  'Atomically projects one emitted candidate from a succeeded run into company_findings under an organization/semantic-root row lock. Preserves silences, applies declared known-problem escalation, atomically increments occurrence_count, and refuses stale backfills.';
comment on function public.project_management_pattern_run(uuid,uuid) is
  'Bounded one-call projection for a succeeded run. Projects every emit candidate, preserves suppressed/abstained state, and resolves active roots only from explicit newer successful-normal absent reconciliation receipts.';

insert into public.applied_migrations (version, description)
values (
  '0390',
  'Management-company Finding Patterns evidence plane: versioned nullable property comparison-profile overrides; leased/fenced durable runs with exact portfolio snapshots and quality/cost/performance receipts; immutable cohort/member, raw+normalized metric, deterministic check/candidate, and candidate-property occurrence evidence; company_findings remains the mutable projection with semantic root/scope/routing/version metadata and an atomic silence-aware, non-regressing projection RPC. Service-role-only with explicit browser deny policies.'
)
