...
-- 0339 (a sibling workstream, already applied in prod) added
-- pms_feed_catalog.target_table -> pms_table_schemas(table_name). That
-- reference is to a MUTABLE natural key, and the two updates below rename it,
-- so the plain FK aborts this migration with a foreign-key violation. Neither
-- branch could see the other; the merge is where it surfaces.
--
-- Recreate it ON UPDATE CASCADE, which is the honest semantics: renaming a
-- target table should carry its catalog row along, not block the rename. The
-- guard makes this a no-op on a database where 0339 has not been applied.
do $$
begin
  if exists (
    select 1 from pg_constraint where conname = 'pms_feed_catalog_target_table_fkey'
  ) then
    alter table public.pms_feed_catalog
      drop constraint pms_feed_catalog_target_table_fkey;
    alter table public.pms_feed_catalog
      add constraint pms_feed_catalog_target_table_fkey
      foreign key (target_table)
      references public.pms_table_schemas(table_name)
      on update cascade;
  end if;
end $$;
