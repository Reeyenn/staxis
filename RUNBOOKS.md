...
## Audit-02 post-push defects (live commit 487eb17)

The audit-02 ship landed on `main` but the first post-deploy smoke run flagged two issues:

1. **Tests workflow red on the merge commit** — pre-existing `tsc` errors in `src/lib/__tests__/service-hostname-allowlist.test.ts` (NODE_ENV readonly under newer @types/node). Fix: a later main commit added a `mutEnv` cast pattern; no action needed today, but the lesson is to run `npx tsc --noEmit` locally before merging.

2. **`webhook-dedup-purge` cron used `.select('1')`** — `'1'` isn't a column on any of the three dedup tables, so the count metric was always `-1` and the daily run looked broken. Fix landed in this branch: per-table PK passed as `countColumn` (`message_sid` for Twilio, `event_id` for Sentry + Stripe).
